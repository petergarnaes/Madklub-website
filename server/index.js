...
if (process.env.NODE_ENV === "development") {
  ...
  app.get(/(.+graphiql.+\.(js)$|.+api_editor_style\.css$)/, function(
    req,
    res,
    next
  ) {
...
if (process.env.NODE_ENV === "production") {
  ...
  chunkMapManifest = JSON.parse(
    fs.readFileSync("./dist/public/chunk-map-manifest.json")
  );
...
// -----------------------------------------------------------------------------
app.use("/api_editor", (req, res) =>
  res.send(fs.readFileSync("./dist/public/api_editor.html").toString())
);
app.use(
  "/graphql",
...
async function handleRender(req, res) {
  ...
  const client = new ApolloClient(options2);
  ...
  const apolloInitialState = client.cache.extract();
...
function renderFullPage(
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    preloads +=
      '<link rel="preload" href="/public/' +
      manifestFullName +
      '" as="script">\n';
    prefetches +=
      '<link rel="prefetch" href="/public/' + manifestFullName + '">\n';
    manifestDecl =
      '<script type="application/javascript" src="/public/' +
      manifestFullName +
      '"></script>';
    mainFullName = chunkMapManifest["main.js"];
    preloads +=
      '<link rel="preload" href="/public/' + mainFullName + '" as="script">\n';
    ...
    preloads +=
      ...
      '" as="script">\n';
    prefetches +=
      '<link rel="prefetch" href="/public/' + vendorFullName + '">\n';
    vendorDecl =
      '<script type="application/javascript" src="/public/' +
      vendorFullName +
      '"></script>';
    registered_components.forEach(route => {
      ...
      preloads +=
        ...
        '" as="script">\n';
      prefetches +=
        '<link rel="prefetch" href="/public/' + chunkFullName + '">\n';
    });
  }
        ...
        <html>
            ...
            <body>
                ...
                <script>
                    ...
                    window.__REGISTERED_COMPONENTS__ = ${JSON.stringify(
                      registered_components
                    )}
                </script>
                ${manifestDecl}
