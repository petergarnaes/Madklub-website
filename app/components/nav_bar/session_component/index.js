...
const SessionComponent = ({ data, display_name, history }) => {
  ...
  if (me.id === me.kitchen.admin.id) {
    ...
    AdminSettings = (
      <MenuItem eventKey="admin_settings" onSelect={goTo(history)}>
        Admin Indstillinger
        <Glyphicon style={{ paddingLeft: 1 + "em" }} glyph="cutlery" />
      </MenuItem>
    );
    AccountingOption = (
      <MenuItem eventKey="accounting" onSelect={goTo(history)}>
        Regnskabs værktøj
        <Glyphicon style={{ paddingLeft: 1 + "em" }} glyph="piggy-bank" />
      </MenuItem>
    );
  }
  return (
    <NavDropdown title={me.display_name} id="basic-nav-dropdown">
      ...
      <MenuItem eventKey="user_settings" onSelect={goTo(history)}>
        Indstillinger
        <Glyphicon style={{ paddingLeft: 1 + "em" }} glyph="cog" />
      </MenuItem>
      <MenuItem divider />
